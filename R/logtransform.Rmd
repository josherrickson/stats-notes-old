...
Therefore a 1-unit increase in `disp` is associated with a \(\textrm{exp}(`r round(m$coef[2], 3)`) = `r round(exp(m$coef[2]), 3)`\) multiplicative change in `drat`.

To test this, we predict the ratio in predicted outcome with some values of `disp`, and that value increased by 1. **Note**: We exponentiate the
